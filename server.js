...
app.patch('/api/me', authenticateToken, async (req, res) => {
    const { username, email, bio, avatarData } = req.body;   // Desestruturação correta
    ...
    try {
        ...
        console.error(error);
        res.status(400).json({ 
            error: 'Erro ao atualizar perfil. Username ou email já podem estar em uso.' 
        });
    }
});
